...
from symplyphysics import (units, expr_to_quantity, Quantity, Symbol, Function,
                           print_expression, angle_type, CoordinateSystem,
                           Vector, validate_input_symbols,
...
centripetal_acceleration = Symbol("centripetal_acceleration",
                                  units.acceleration)
...
velocity_horisontal = velocity_def.definition.rhs.subs(
    velocity_def.moving_time, time).subs(velocity_def.movement(time),
                                         curve_radius_horisontal).doit()
velocity_vertical = velocity_def.definition.rhs.subs(
    velocity_def.moving_time, time).subs(velocity_def.movement(time),
                                         curve_radius_vertical).doit()
...
radial_unit_vector = Vector(
    [cos(alpha(time)), sin(alpha(time))], cartesian_coordinates)
tangential_unit_vector = Vector(
    [-sin(alpha(time)), cos(alpha(time))], cartesian_coordinates)
...
acceleration_horisontal = acceleration_def.definition.rhs.subs(
    acceleration_def.time, time)
acceleration_horisontal = acceleration_horisontal.subs(
    acceleration_def.velocity(time), velocity_horisontal).doit()
acceleration_vertical = acceleration_def.definition.rhs.subs(
    acceleration_def.time, time)
acceleration_vertical = acceleration_vertical.subs(
    acceleration_def.velocity(time), velocity_vertical).doit()
...
tangential_acceleration_magnitude = curve_radius * Derivative(
    alpha(time), (time, 2))
...
tangential_acceleration_component = dot_vectors(acceleration_vector,
                                                tangential_unit_vector)
radial_acceleration_component = dot_vectors(acceleration_vector,
                                            radial_unit_vector)
assert expr_equals(tangential_acceleration_component,
                   tangential_acceleration_magnitude)
...
angular_velocity_applied = angular_velocity_applied.subs(
    angular_velocity_def.angle_function(time), alpha(time))
...
@validate_input_symbols(linear_velocity_=linear_velocity,
                        curve_radius_=curve_radius)
@validate_output_symbol(centripetal_acceleration)
def calculate_acceleration(linear_velocity_: Quantity,
                           curve_radius_: Quantity) -> Quantity:
    solved = solve(law, centripetal_acceleration,
