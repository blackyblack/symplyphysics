...
constant_acceleration = Symbol("constant_acceleration",
                               units.acceleration,
                               constant=True)
...
law = Eq(
    distance(movement_time), initial_velocity * movement_time +
    constant_acceleration * movement_time**2 / 2)
...
derived_law = Eq(distance(movement_time),
                 constant_accelerated_movement_function)

# Prove that constant_accelerated_movement_function equals to law.rhs, given C1 = initial_velocity, C2 = 0
assert (expr_equals(derived_law.rhs.subs({
    'C1': initial_velocity,
    'C2': 0
}), law.rhs))
...
def calculate_distance(initial_velocity_: Quantity, acceleration_: Quantity,
    ...
    result_expr = solve(law, distance(movement_time),
                        dict=True)[0][distance(movement_time)]
