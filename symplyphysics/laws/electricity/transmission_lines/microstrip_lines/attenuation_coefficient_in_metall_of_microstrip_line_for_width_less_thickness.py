...
expression_3 = Symbol("expression_3", dimensionless)
...
def calculate_attenuation_coefficient(surface_resistance_: Quantity,
    ...
    if thickness_of_substrate_.scale_factor >= 2 * pi * effective_width_.scale_factor:
        expression = 1 + (thickness_of_substrate / effective_width) * (1 + 1.25 * strip_thickness / (pi * width) + 1.25 * log(4 * pi * width / strip_thickness) / pi)
    else:
        expression = 1 + (thickness_of_substrate / effective_width) * (1 - 1.25 * strip_thickness / (pi * thickness_of_substrate) + 1.25 * log(2 * thickness_of_substrate / strip_thickness) / pi)
    law_applied = law.subs({
        expression_3: expression
    })

    result_expr = solve(law_applied, attenuation_coefficient, dict=True)[0][attenuation_coefficient]
