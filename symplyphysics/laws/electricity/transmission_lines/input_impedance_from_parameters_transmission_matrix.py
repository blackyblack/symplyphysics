...
law = Eq(input_impedance, (parameter_voltage_to_voltage * load_resistance + parameter_impedance) / (parameter_conductance * load_resistance + parameter_current_to_current))
...
def calculate_input_impedance(load_resistance_: Quantity, parameters_: tuple[tuple[float, Quantity], tuple[Quantity, float]]) -> Quantity:
    assert_equivalent_dimension(parameters_[0][1], f"parameters_[{0}][{1}]", "calculate_input_impedance", units.impedance)
    assert_equivalent_dimension(parameters_[1][0], f"parameters_[{1}][{0}]", "calculate_input_impedance", units.conductance)
