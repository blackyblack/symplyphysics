...
from symplyphysics import (QuantityVector, Symbol, add_cartesian_vectors, scale_vector, units, Quantity, validate_input, validate_output)
...
def conductivity_current_density_vector_law(magnetic_intensity: VectorField, electric_induction: VectorField) -> Vector:
    ...
    electric_induction_time_derivative = [diff(c, time) for c in electric_induction_space.components]
    electric_induction_time_derivative_vector = Vector(electric_induction_time_derivative, electric_induction.coordinate_system)
    return add_cartesian_vectors(rotor_magnetic_intensity_space, scale_vector(-1, electric_induction_time_derivative_vector))


def magnetic_intensity_rotor_law(electric_induction: VectorField, conductivity_current_density: Vector) -> VectorField:
    electric_induction_space = electric_induction.apply_to_basis()
    electric_induction_time_derivative = [diff(c, time) for c in electric_induction_space.components]
    electric_induction_time_derivative_vector = Vector(electric_induction_time_derivative, electric_induction.coordinate_system)
    sum_vectors = add_cartesian_vectors(conductivity_current_density, electric_induction_time_derivative_vector)
    return VectorField.from_vector(sum_vectors)


def electric_induction_time_derivative_law(magnetic_intensity: VectorField, conductivity_current_density: Vector) -> VectorField:
    rotor_magnetic_intensity = curl_operator(magnetic_intensity)
    rotor_magnetic_intensity_space = rotor_magnetic_intensity.apply_to_basis()
    sub_vectors = add_cartesian_vectors(rotor_magnetic_intensity_space, scale_vector(-1, conductivity_current_density))
...
def calculate_conductivity_current_density_at_point(magnetic_intensity_: VectorField,
    ...
    result_vector = conductivity_current_density_vector_law(magnetic_intensity_, electric_induction_)
    base_scalars = result_vector.coordinate_system.coord_system.base_scalars()
    return QuantityVector.from_base_vector(result_vector, subs={base_scalars[0]: cartesian_point_[0], base_scalars[1]: cartesian_point_[1], base_scalars[2]: cartesian_point_[2], time: time_})
