...
two_currents_law = kirchhoff_law.law.subs(kirchhoff_law.currents_total,
                                          2).doit()
...
capacitor_current_applied = solve(two_currents_applied,
                                  capacitor_current(time),
...
two_voltages_law = kirchhoff_law_2.law.subs(kirchhoff_law_2.voltages_total,
                                            2).doit()
...
inductor_voltage_applied = solve(two_voltages_applied,
                                 inductor_voltage(time),
...
capacitor_charge_applied = solve(capacitor_charge_law,
                                 charge_definition.charge(time),
                                 dict=True)[0][charge_definition.charge(time)]

## I_c(t) = C * U_c'(t)
capacitor_current_law = charge_definition.definition.subs(
    charge_definition.time, time)
capacitor_current_law = capacitor_current_law.subs(
    charge_definition.charge(time), capacitor_charge_applied)
## I_c'(t) = C * U_c"(t)
capacitor_current_law_derivative = Eq(
    Derivative(capacitor_current_law.lhs, time),
...
inductor_voltage_law = inductor_voltage_law.subs({
    induction_voltage_definition.inductance:
        inductance,
    induction_voltage_definition.self_induction_voltage(time):
        capacitor_voltage(time),
    induction_voltage_definition.current(time):
        charge_definition.current(time)
})

derived_law = [inductor_voltage_law, capacitor_current_law_derivative]

## U"(t) = - 1/LC * U(t)
capacitor_voltage_solved = solve(
    derived_law,
...
voltage_diff_solved = voltage_diff_eq.subs(
    {capacitor_voltage(time): voltage_diff_solution})

assert simplify(voltage_diff_solved.lhs - voltage_diff_solved.rhs) == 0

# 6. Derive period from frequency
period_law = period_definition.definition.subs(
    period_definition.circular_frequency, frequency)
period_solved = solve(period_law, period_definition.period,
...
def calculate_oscillation_period(inductance_: Quantity,
                                 capacitance_: Quantity) -> Quantity:
    result_period_expr = solve(law, oscillation_period,
                               dict=True)[0][oscillation_period]
