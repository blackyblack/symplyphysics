...
resistor_voltage_applied = solve(three_voltages_applied, resistor_voltage(time), dict=True)[0][resistor_voltage(time)]
...
resistor_ohm_eq = ohms_law.law.subs({
    ...
    ohms_law.resistance: resistance, ohms_law.current: charge_definition.current(time)})
...
solved_charge_function = solve(derived_law, (capacitor_voltage(time), charge_definition.current(time), charge_definition.charge(time)), dict=True)[0][charge_definition.charge(time)]
...
capacitor_charge_function = initial_voltage * capacitance - resistance * capacitance * Derivative(charge_definition.charge(time), time)
assert simplify(charge_diff_eq.rhs - capacitor_charge_function) == 0

## 4. Convert charge to capacitor voltage
capacitor_voltage_solved = solve(capacitance_eq, charge_definition.charge(time), dict=True)[0][charge_definition.charge(time)]
...
def calculate_capacitor_voltage(initial_voltage_: Quantity, capacitance_: Quantity, resistance_: Quantity, time_: Quantity) -> Quantity:
    capacitor_voltage_expr = solve(law, capacitor_voltage(time), dict=True)[0][capacitor_voltage(time)]
