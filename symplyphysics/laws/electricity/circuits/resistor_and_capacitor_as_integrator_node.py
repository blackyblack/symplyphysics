...
law = Eq(capacitor_voltage(time),
         initial_voltage * (1 - exp(-time / (capacitance * resistance))))
...
three_voltages_law = kirchhoff_law_2.law.subs(kirchhoff_law_2.voltages_total,
                                              3).doit()
...
resistor_voltage_applied = solve(three_voltages_applied,
                                 resistor_voltage(time),
...
solved_charge_function = solve(
    derived_law, (capacitor_voltage(time), charge_definition.current(time),
...
capacitor_voltage_solved = solve(capacitance_eq,
                                 charge_definition.charge(time),
                                 dict=True)[0][charge_definition.charge(time)]
voltage_diff_eq = charge_diff_eq.subs(charge_definition.charge(time),
                                      capacitor_voltage_solved)
...
@validate_input_symbols(initial_voltage_=initial_voltage,
                        capacitance_=capacitance,
                        resistance_=resistance,
                        time_=time)
@validate_output_symbol(capacitor_voltage)
def calculate_capacitor_voltage(initial_voltage_: Quantity,
                                capacitance_: Quantity, resistance_: Quantity,
