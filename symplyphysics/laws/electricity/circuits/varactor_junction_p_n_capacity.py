...
junction_transition_capacity_without_bias_voltage = Symbol("junction_transition_capacity_without_bias_voltage", units.capacitance)
...
law = Eq(junction_capacitance, junction_transition_capacity_without_bias_voltage / (1 - voltage / material_parameter)**doping_coefficient)


@validate_input(junction_transition_capacity_without_bias_voltage_=junction_transition_capacity_without_bias_voltage,
                voltage_=voltage,
                material_parameter_=material_parameter,
                doping_coefficient_=doping_coefficient)
@validate_output(junction_capacitance)
def calculate_junction_capacitance(junction_transition_capacity_without_bias_voltage_: Quantity, voltage_: Quantity,
                                                  material_parameter_: float, doping_coefficient_: Quantity) -> Quantity:
    result_expr = solve(law, junction_capacitance, dict=True)[0][junction_capacitance]
    result_expr = result_expr.subs({
        junction_transition_capacity_without_bias_voltage: junction_transition_capacity_without_bias_voltage_,
        voltage: voltage_,
