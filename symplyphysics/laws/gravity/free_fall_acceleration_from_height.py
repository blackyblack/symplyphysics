...
law = Eq(
    free_fall_acceleration, gravitational_constant * planet_mass /
    (planet_radius + height_above_surface)**2)
...
gravitational_force = gravity_law.law.rhs.subs({
    gravity_law.first_object_mass:
        planet_mass,
    gravity_law.distance_between_mass_centers:
        planet_radius + height_above_surface
})
...
def calculate_acceleration(planet_mass_: Quantity, planet_radius_: Quantity,
    ...
    result_accel_expr = solve(law, free_fall_acceleration,
                              dict=True)[0][free_fall_acceleration]
