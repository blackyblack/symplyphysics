...
from symplyphysics import (units, expr_to_quantity, Quantity, Symbol,
                           print_expression, angle_type, Vector,
                           sympy_vector_from_vector, vector_rebase,
...
def calculate_work(force_: Quantity, distance_: Quantity,
                   force_angle: Quantity | float,
    ...
    coordinates_cartesian = coordinates_transform(
        coordinates_polar, CoordinateSystem.System.CARTESIAN)
    #HACK: sympy angles are always in radians
    force_angle_radians = force_angle if isinstance(
        force_angle, numbers.Number) else force_angle.scale_factor
    distance_angle_radians = distance_angle if isinstance(
        distance_angle, numbers.Number) else distance_angle.scale_factor
    ...
    distance_vector = Vector([distance_, distance_angle_radians],
                             coordinates_polar)
    force_vector_cartesian = vector_rebase(force_vector, coordinates_cartesian)
    distance_vector_cartesian = vector_rebase(distance_vector,
                                              coordinates_cartesian)
    force_vector_cartesian_sympy = sympy_vector_from_vector(
        force_vector_cartesian)
    distance_vector_cartesian_sympy = sympy_vector_from_vector(
        distance_vector_cartesian)
