...
class CoordinateSystem:
    ...
    def __init__(self,
                 coord_system_type=System.CARTESIAN,
                 inner: CoordSys3D = None):
        self._coord_system_type = coord_system_type
        if inner is None:
            self._coord_system = CoordSys3D(
                CoordinateSystem.random_name(),
                variable_names=CoordinateSystem.system_to_base_scalars(
                    coord_system_type))
            return
    ...
    def transformation_to_system(self, coord_system_type: System):
        ...
        coord_name_from = self.system_to_transformation_name(
            self._coord_system_type)
...
def coordinates_transform(
        ...
        coord_system_type=CoordinateSystem.System.CARTESIAN
) -> CoordinateSystem:
    if coord_system_type == None:
        coord_system_type = self.coord_system_type
    new_coord_system = self.coord_system.create_new(
        CoordinateSystem.random_name(),
        variable_names=CoordinateSystem.system_to_base_scalars(
            coord_system_type),
        transformation=None)
...
def coordinates_rotate(self: CoordinateSystem, angle, axis) -> CoordinateSystem:
    ...
    return CoordinateSystem(
        self.coord_system_type,
        self.coord_system.orient_new_axis(CoordinateSystem.random_name(), angle,
                                          axis))
