...
def vector_from_sympy_vector(sympy_vector_: Any,
    ...
    if len(coord_system_set) > 1:
        ...
        raise TypeError(
            f"Different coordinate systems in expression: {str(coord_sys_names)}"
        )
    if len(coord_system_set) > 1:
...
def sympy_vector_from_vector(vector_: Vector) -> SympyVector:
    ...
    for idx in range(min(len(base_vectors), len(vector_.components))):
        result_vector = result_vector + base_vectors[idx] * vector_.components[
            idx]
    return result_vector
...
def vector_rebase(vector_: Vector,
                  coordinate_system: CoordinateSystem = None) -> Vector:
...
def _extended_express(vector_: Vector, system_to: CoordinateSystem = None):
    if vector_.coordinate_system.coord_system_type != system_to.coord_system_type:
        new_scalars = list(
            vector_.coordinate_system.transformation_to_system(
                system_to.coord_system_type))
        ...
        for i, scalar in enumerate(
                vector_.coordinate_system.coord_system.base_scalars()):
            new_component = 0 if i >= len(
                vector_.components) else vector_.components[i]
