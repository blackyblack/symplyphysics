...
from symplyphysics.laws.astronomy import relative_aperture_of_telescope as relative_aperture_law
...
def test_basic_relative_aperture(test_args: Args) -> None:
    result = relative_aperture_law.calculate_relative_aperture(test_args.lens_diameter, test_args.focal_length_lens)
...
def test_bad_lens_diameter(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        relative_aperture_law.calculate_relative_aperture(lens_diameter, test_args.focal_length_lens)
...
def test_bad_focal_length_lens(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        relative_aperture_law.calculate_relative_aperture(test_args.lens_diameter, focal_length_lens)
    with raises(TypeError):
