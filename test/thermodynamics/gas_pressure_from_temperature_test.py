...
## the changed pressure will be 108744 pascal.
...
def test_basic_pressure(test_args: Args) -> None:
    result = pressure_law.calculate_pressure(test_args.standard_pressure,
...
def test_bad_pressure(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        pressure_law.calculate_pressure(standard_pressure, test_args.thermal_coefficient,
            test_args.temperature)
    with raises(TypeError):
        pressure_law.calculate_pressure(100, test_args.thermal_coefficient,
            test_args.temperature)
...
def test_bad_thermal_coefficient(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        pressure_law.calculate_pressure(test_args.standard_pressure, thermal_coefficient,
            test_args.temperature)
    with raises(TypeError):
        pressure_law.calculate_pressure(test_args.standard_pressure, 100,
            test_args.temperature)
...
def test_bad_temperature(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        pressure_law.calculate_pressure(test_args.standard_pressure,
