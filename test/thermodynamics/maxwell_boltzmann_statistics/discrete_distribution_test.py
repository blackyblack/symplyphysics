...
def test_law(test_args: Args) -> None:
    result = discrete_distribution.calculate_particle_count_in_microstate(test_args.n, test_args.e, test_args.t, test_args.z)
...
def test_bad_particle_count(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        discrete_distribution.calculate_particle_count_in_microstate(nb, test_args.e, test_args.t, test_args.z)


def test_bad_energy(test_args: Args) -> None:
    eb = Quantity(1.0 * units.coulomb)
    with raises(errors.UnitsError):
        discrete_distribution.calculate_particle_count_in_microstate(test_args.n, eb, test_args.t, test_args.z)
    with raises(TypeError):
        discrete_distribution.calculate_particle_count_in_microstate(test_args.n, 100, test_args.t, test_args.z)


def test_bad_temperature(test_args: Args) -> None:
    tb = Quantity(1.0 * units.coulomb)
    with raises(errors.UnitsError):
        discrete_distribution.calculate_particle_count_in_microstate(test_args.n, test_args.e, tb, test_args.z)
    with raises(TypeError):
        discrete_distribution.calculate_particle_count_in_microstate(test_args.n, test_args.e, 100, test_args.z)


def test_bad_partition_function(test_args: Args) -> None:
    zb = Quantity(1.0 * units.coulomb)
    with raises(errors.UnitsError):
        discrete_distribution.calculate_particle_count_in_microstate(test_args.n, test_args.e, test_args.t, zb)
