...
def test_args_fixture():
    ...
    Args = namedtuple("Args", ["initial_pressure", "atomic_weight", "final_height", "initial_height", "temperature"])
    return Args(
...
def test_bad_atomic_weight(test_args):
    ...
    with raises(errors.UnitsError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, bad_atomic_weight,
                                                       test_args.final_height, test_args.initial_height, test_args.temperature)
    with raises(TypeError):
...
def test_bad_height(test_args):
    ...
    with raises(errors.UnitsError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, test_args.atomic_weight,
                                                                bad_height, test_args.initial_height, test_args.temperature)
    with raises(TypeError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, test_args.atomic_weight,
                                                                100, test_args.initial_height, test_args.temperature)
    with raises(errors.UnitsError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, test_args.atomic_weight,
                                                                test_args.final_height, bad_height, test_args.temperature)
    with raises(TypeError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, test_args.atomic_weight,
                                                                test_args.final_height, 100, test_args.temperature)

def test_bad_temperature(test_args):
    bad_temperature = Quantity(1 * units.meter)
    with raises(errors.UnitsError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, test_args.atomic_weight,
                                                                test_args.final_height, test_args.initial_height, bad_temperature)
    with raises(TypeError):
        pressure_of_ideal_gas.calculate_final_pressure(test_args.initial_pressure, test_args.atomic_weight,
                                                                test_args.final_height, test_args.initial_height, 100)
