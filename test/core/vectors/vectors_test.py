...
def test_basic_extract_coord_system_from_sympy_vector(test_args):
    coord_system = extract_coord_system_from_sympy_vector(test_args.C.coord_system.x + test_args.C.coord_system.y)
    assert coord_system == test_args.C.coord_system
...
def test_different_coord_systems_extract_coord_system_from_sympy_vector(test_args):
    ...
    with raises(TypeError):
        extract_coord_system_from_sympy_vector(test_args.C.coord_system.x + 2 * C1.phi)
    with raises(TypeError):
        extract_coord_system_from_sympy_vector(test_args.C.coord_system.x * C1.i)
"""
...
def test_basic_sympy_to_array_conversion(test_args):
    vector = vector_from_sympy_vector(test_args.C.coord_system.i + 2 * test_args.C.coord_system.j, test_args.C)
    assert vector.components == [1, 2, 0]
    assert vector.coordinate_system == test_args.C

def test_order_sympy_to_array_conversion(test_args):
    vector = vector_from_sympy_vector(2 * test_args.C.coord_system.j + test_args.C.coord_system.i, test_args.C)
    assert vector.components == [1, 2, 0]

def test_skip_dimension_sympy_to_array_conversion(test_args):
    vector = vector_from_sympy_vector(test_args.C.coord_system.i + 2 * test_args.C.coord_system.k, test_args.C)
...
def test_rotate_coordinates_array_to_sympy_conversion(test_args):
    ...
    theta = symbols("theta")
    B = CoordinateSystem(test_args.C.coord_system_type, test_args.C)
    B.rotate(theta, B.coord_system.k)
...
def test_multiple_coord_systems_sympy_to_array_conversion(test_args):
    ...
    with raises(TypeError):
        vector_from_sympy_vector(test_args.C.coord_system.i + C1.coord_system.r * test_args.C.coord_system.k, C1)
