...
def test_basic_add_vectors(test_args):
    ...
    assert add_cartesian_vectors(Vector([test_args.C.coord_system.x, 2]), Vector([test_args.C.coord_system.x, 2])).components == [2 * test_args.C.coord_system.x, 4]
    assert add_cartesian_vectors(Vector([test_args.C.coord_system.x, 2]), Vector([test_args.C.coord_system.y, 2])).components == [test_args.C.coord_system.x + test_args.C.coord_system.y, 4]
...
def test_strings_add_vectors():
    with raises(TypeError):
        add_cartesian_vectors(Vector(["hello", 2]), Vector([2, 1]))
    assert add_cartesian_vectors(Vector(["hello", 2]), Vector([" world"])).components == ["hello world", 2]
...
def test_cylindrical_scale_vector(test_args):
    ...
    result_vector_magnitude = sqrt(scaled_vector1.components[0]**2 + scaled_vector1.components[1]**2)
    ...
    # Verify that magnitude has changed times scalar value, angle hasn't changed    
    initial_vector2_magnitude = sqrt(vector2.components[0]**2 + vector2.components[1]**2 + vector2.components[2]**2)
    result_vector2_magnitude = sqrt(scaled_vector2.components[0]**2 + scaled_vector2.components[1]**2 + scaled_vector2.components[2]**2)
...
def test_basic_magnitude_vectors(test_args):
    ...
    assert vector_magnitude(Vector([test_args.C.coord_system.x, 2])) == sqrt(test_args.C.coord_system.x**2 + 4)
