...
def test_applied_point() -> None:
    ...
    x, y, z = cart_sys.base_scalars
    assert expr_equals(cart_pt[x], 0)
    assert expr_equals(cart_pt[y], -1)
    assert expr_equals(cart_pt[z], 4)
    ...
    rho, phi, z = cyl_sys.base_scalars
    assert expr_equals(cyl_pt[rho], 1)
    assert expr_equals(cyl_pt[phi], pi / 3)
    assert expr_equals(cyl_pt[z], -3)
    ...
    r, theta, phi = sph_sys.base_scalars
    assert expr_equals(sph_pt[r], 5)
    assert expr_equals(sph_pt[theta], pi / 2)
    assert expr_equals(sph_pt[phi], pi)
