...
def test_args_fixture():
    ...
    energy_func = ((planck_constant/2/pi)**2) * (((propagation_vec_axis**2) / 2 / mass_1) + (period_length/mass_2) * propagation_vec_axis**3)
    Args = namedtuple("Args", ["energy_func", "propagation_vec_axis"])
    return Args(energy_func=energy_func,
                propagation_vec_axis=propagation_vec_axis)


def test_basic_mass(test_args):
    result = ef_mass_el.calculate_mass(test_args.energy_func, test_args.propagation_vec_axis)
    ...
    assert result_current == approx(5.91249e-9, 0.00001)


def test_bad_propagation_vec(test_args):
    propagation_vec_axis = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        ef_mass_el.calculate_mass(test_args.energy_func, propagation_vec_axis)
