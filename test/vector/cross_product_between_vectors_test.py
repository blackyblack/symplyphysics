...
from symplyphysics.laws.vector import cross_product_between_vectors as cross_law
...
def test_basic_law(test_args):
    result = cross_law.cross_product_norm_law(test_args.a, test_args.b)
    assert result.evalf(3) == approx(2.45, 1e-3)


def test_basic_law_perpendicular_vectors(test_args):
    result = cross_law.cross_product_norm_law(test_args.a, test_args.c)
    assert result.evalf(3) == approx(5.66, 1e-3)


def test_basic_law_parallel_vectors(test_args):
    result = cross_law.cross_product_norm_law(test_args.a, test_args.d)
...
def test_bad_dimensions_count(_test_args):
    ...
    with raises(ValueError):
        cross_law.cross_product_norm_law(a_bad, b_bad)
