...
from symplyphysics.laws.astronomy.stellar_evolution import luminosity_of_sun_in_past_from_luminosity_in_present as luminosity_law
...
def test_args_fixture() -> Args:
    ...
    time = Quantity(1e9 * units.common_year)
...
def test_basic_luminosity_past(test_args: Args) -> None:
    ...
    assert_equal(result, 0.762 * units.joule / units.second)
...
def test_bad_time(test_args: Args) -> None:
    ...
    with raises(TypeError):
        luminosity_law.calculate_luminosity_past(test_args.luminosity_present, 100)
