...
def test_basic_circulation(test_args):
    field = VectorField(lambda point: point.y, lambda point: 0,
    ...
    result_expr = circulation_def.calculate_circulation(field, surface, 0, 1, 0,
                                                        pi / 2)
...
def test_two_parameters_circulation(test_args):
    ...
    result_expr = circulation_def.calculate_circulation(field, cone, 0, 1, 0,
                                                        2 * pi)
...
def test_gravitational_field_is_conservative(test_args):
    field = VectorField(lambda point: -point.x / _distance(point)**3,
                        lambda point: -point.y / _distance(point)**3,
                        lambda point: -point.z / _distance(point)**3,
                        test_args.C)
...
def test_force_field_circulation(test_args):
    ...
    result_expr = circulation_def.calculate_circulation(
        field, surface, 1 * test_args.radius_unit, 2 * test_args.radius_unit, 0,
        pi / 2)
