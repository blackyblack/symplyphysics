...
Args = namedtuple("Args", [
    "first_constant_of_gas", "second_constant_of_gas", "pressure", "electric_intensity",
])
...
def test_bad_first_constant_of_gas(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_coefficient_of_volumetric_ionization(first_constant_of_gas,
            test_args.second_constant_of_gas, test_args.pressure,
            test_args.electric_intensity)
    with raises(TypeError):
        coefficient_law.calculate_coefficient_of_volumetric_ionization(100,
            test_args.second_constant_of_gas, test_args.pressure,
            test_args.electric_intensity)
...
def test_bad_second_constant_of_gas(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_coefficient_of_volumetric_ionization(test_args.first_constant_of_gas,
            second_constant_of_gas, test_args.pressure, test_args.electric_intensity)
    with raises(TypeError):
        coefficient_law.calculate_coefficient_of_volumetric_ionization(test_args.first_constant_of_gas,
            100, test_args.pressure, test_args.electric_intensity)
