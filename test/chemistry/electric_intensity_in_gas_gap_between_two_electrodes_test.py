...
Args = namedtuple("Args", ["coordinate", "distance_between_electrodes", "voltage_between_electrodes"])
...
def test_basic_electric_intensity(test_args: Args) -> None:
    result = intensity_law.calculate_electric_intensity(test_args.coordinate, test_args.distance_between_electrodes,
...
def test_bad_distance_between_electrodes(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        intensity_law.calculate_electric_intensity(test_args.coordinate, distance_between_electrodes,
            test_args.voltage_between_electrodes)
    with raises(TypeError):
...
def test_bad_voltage_between_electrodes(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        intensity_law.calculate_electric_intensity(test_args.coordinate, test_args.distance_between_electrodes,
            voltage_between_electrodes)
    with raises(TypeError):
        intensity_law.calculate_electric_intensity(test_args.coordinate, test_args.distance_between_electrodes, 100)
