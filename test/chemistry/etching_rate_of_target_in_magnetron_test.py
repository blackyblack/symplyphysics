...
Args = namedtuple("Args", ["ion_current_density", "molar_mass_of_target_atom", "sputtering_coefficient", "target_density"])
...
def test_basic_etching_rate(test_args: Args) -> None:
    result = rate_law.calculate_etching_rate(test_args.ion_current_density, test_args.molar_mass_of_target_atom, test_args.sputtering_coefficient, test_args.target_density)
...
def test_bad_ion_current_density(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        rate_law.calculate_etching_rate(ion_current_density, test_args.molar_mass_of_target_atom, test_args.sputtering_coefficient, test_args.target_density)
    with raises(TypeError):
        rate_law.calculate_etching_rate(100, test_args.molar_mass_of_target_atom, test_args.sputtering_coefficient, test_args.target_density)


def test_bad_molar_mass_of_target_atom(test_args: Args) -> None:
    molar_mass_of_target_atom = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        rate_law.calculate_etching_rate(test_args.ion_current_density, molar_mass_of_target_atom, test_args.sputtering_coefficient, test_args.target_density)
    with raises(TypeError):
...
def test_bad_sputtering_coefficient(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        rate_law.calculate_etching_rate(test_args.ion_current_density, test_args.molar_mass_of_target_atom, sputtering_coefficient, test_args.target_density)


def test_bad_target_density(test_args: Args) -> None:
    target_density = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        rate_law.calculate_etching_rate(test_args.ion_current_density, test_args.molar_mass_of_target_atom, test_args.sputtering_coefficient, target_density)
    with raises(TypeError):
        rate_law.calculate_etching_rate(test_args.ion_current_density, test_args.molar_mass_of_target_atom, test_args.sputtering_coefficient, 100,)
