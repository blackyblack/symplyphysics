...
from symplyphysics.reorganized.thermodynamics.heat_transfer.heat_diffusion import general_equation_in_one_dimension as heat_equation

# Description
## The values of the parameters are that of carbon monoxide (CO).

Args = namedtuple("Args", "rho cp k q T")
...
def test_args_fixture() -> Args:
    ...
    return Args(rho=rho, cp=cp, k=k, q=q, T=T)


def test_law(test_args: Args) -> None:
    values = {
        heat_equation.medium_density: test_args.rho,
        heat_equation.medium_specific_isobaric_heat_capacity: test_args.cp,
