...
from symplyphysics.reorganized.electromagnetism.magnetostatics.straight_wire import magnetic_induction_of_linear_conductor_of_finite_length as induction_law
...
Args = namedtuple("Args",
    ["absolute_permeability", "current", "first_angle", "second_angle", "distance"])


@fixture(name="test_args")
def test_args_fixture() -> Args:
    relative_permeability_ = 10
...
def test_basic_induction(test_args: Args) -> None:
    result = induction_law.calculate_induction(test_args.absolute_permeability, test_args.current,
...
def test_bad_current(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        induction_law.calculate_induction(test_args.absolute_permeability, current,
            test_args.first_angle, test_args.second_angle, test_args.distance)
    with raises(TypeError):
