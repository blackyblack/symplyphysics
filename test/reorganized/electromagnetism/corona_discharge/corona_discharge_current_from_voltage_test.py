...
from symplyphysics.reorganized.electromagnetism.corona_discharge import corona_discharge_current_from_voltage as current_section_law
...
Args = namedtuple("Args", [
    "gas_coefficient",
    "mobility_of_charged_particles",
    "voltage",
    "corona_discharge_occurrence_voltage",
])
...
def test_basic_current(test_args: Args) -> None:
    result = current_section_law.calculate_current(test_args.gas_coefficient,
...
def test_bad_gas_coefficient(test_args: Args) -> None:
    ...
    with raises(TypeError):
        current_section_law.calculate_current(100, test_args.mobility_of_charged_particles,
            test_args.voltage, test_args.corona_discharge_occurrence_voltage)
...
def test_bad_mobility_of_charged_particles(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_section_law.calculate_current(test_args.gas_coefficient,
            mobility_of_charged_particles, test_args.voltage,
            test_args.corona_discharge_occurrence_voltage)
    with raises(TypeError):
        current_section_law.calculate_current(test_args.gas_coefficient, 100, test_args.voltage,
            test_args.corona_discharge_occurrence_voltage)
...
def test_bad_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_section_law.calculate_current(test_args.gas_coefficient,
            test_args.mobility_of_charged_particles, voltage,
            test_args.corona_discharge_occurrence_voltage)
    with raises(TypeError):
        current_section_law.calculate_current(test_args.gas_coefficient,
            test_args.mobility_of_charged_particles, 100,
            test_args.corona_discharge_occurrence_voltage)
