...
from symplyphysics.reorganized.continuum_mechanics.fluid_mechanics.similarity_parameters import grashof_number_via_fluid_parameters as law
...
def test_basic_grashof_number(test_args: Args) -> None:
    result = law.calculate_grashof_number(
        test_args.beta,
        test_args.t_surf,
        test_args.t_bulk,
        test_args.l,
...
def test_bad_coefficient_of_volume_expansion(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        law.calculate_grashof_number(beta, test_args.t_surf, test_args.t_bulk,
            test_args.l, test_args.mu)
    with raises(TypeError):
        law.calculate_grashof_number(100, test_args.t_surf, test_args.t_bulk,
            test_args.l, test_args.mu)
...
def test_bad_temperature(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        law.calculate_grashof_number(test_args.beta, ts, test_args.t_bulk, test_args.l,
            test_args.mu)
    with raises(errors.UnitsError):
        law.calculate_grashof_number(test_args.beta, test_args.t_surf, ts, test_args.l,
            test_args.mu)
    with raises(TypeError):
        law.calculate_grashof_number(test_args.beta, 100, test_args.t_bulk, test_args.l,
            test_args.mu)
    with raises(TypeError):
        law.calculate_grashof_number(test_args.beta, test_args.t_surf, 100, test_args.l,
            test_args.mu)
...
def test_bad_length(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        law.calculate_grashof_number(test_args.beta, test_args.t_surf, test_args.t_bulk,
            l, test_args.mu)
    with raises(TypeError):
        law.calculate_grashof_number(test_args.beta, test_args.t_surf, test_args.t_bulk,
            100, test_args.mu)
