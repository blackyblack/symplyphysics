...
from symplyphysics.reorganized.continuum_mechanics.fluid_mechanics.hydrostatics import hydrostatic_pressure_via_density_height_and_acceleration as law
...
def test_hydrostatic_pressure(test_args: Args) -> None:
    result = law.calculate_hydrostatic_pressure(test_args.rho, test_args.h, test_args.acceleration)
...
def test_bad_density(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        law.calculate_hydrostatic_pressure(bad_density, test_args.h, test_args.acceleration)
    with raises(TypeError):
        law.calculate_hydrostatic_pressure(100, test_args.h, test_args.acceleration)


def test_bad_depth(test_args: Args) -> None:
    bad_depth = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        law.calculate_hydrostatic_pressure(test_args.rho, bad_depth, test_args.acceleration)
    with raises(TypeError):
        law.calculate_hydrostatic_pressure(test_args.rho, 100, test_args.acceleration)


def test_bad_acceleration(test_args: Args) -> None:
    bad_acceleration = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        law.calculate_hydrostatic_pressure(test_args.rho, test_args.h, bad_acceleration)
    with raises(TypeError):
        law.calculate_hydrostatic_pressure(test_args.rho, test_args.h, 100)
