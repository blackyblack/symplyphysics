...
from symplyphysics.reorganized.optics.geometrical_optics.lenses import optical_power_of_spherical_lens_from_refractive_indices_and_distances as spherical_lens_law
...
def test_bad_distance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        spherical_lens_law.calculate_refraction_index_lens(db, test_args.di, test_args.rl,
            test_args.nm)
    with raises(TypeError):
        spherical_lens_law.calculate_refraction_index_lens(100, test_args.di, test_args.rl,
            test_args.nm)

    with raises(errors.UnitsError):
        spherical_lens_law.calculate_refraction_index_lens(test_args.do, db, test_args.rl,
            test_args.nm)
    with raises(TypeError):
        spherical_lens_law.calculate_refraction_index_lens(test_args.do, 100, test_args.rl,
            test_args.nm)
...
def test_bad_radius(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        spherical_lens_law.calculate_refraction_index_lens(test_args.do, test_args.di, rb,
            test_args.nm)
    with raises(TypeError):
        spherical_lens_law.calculate_refraction_index_lens(test_args.do, test_args.di, 100,
            test_args.nm)
