...
from symplyphysics.reorganized.optics.wave_optics.interference.newton_rings import radius_dark_newton_ring_from_wavelength_order_radius_curvature_refractive_index as radius_law
...
Args = namedtuple("Args",
    ["wavelength", "order_of_ring", "radius_curvature", "refractive_index_between_lens_plate"])
...
def test_args_fixture() -> Args:
    ...
    return Args(wavelength=wavelength,
        order_of_ring=order_of_ring,
        refractive_index_between_lens_plate=refractive_index_between_lens_plate,
        radius_curvature=radius_curvature)


def test_basic_radius(test_args: Args) -> None:
    result = radius_law.calculate_radius(test_args.wavelength, test_args.order_of_ring,
        test_args.refractive_index_between_lens_plate, test_args.radius_curvature)
...
def test_bad_wavelength(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        radius_law.calculate_radius(wavelength, test_args.order_of_ring,
            test_args.refractive_index_between_lens_plate, test_args.radius_curvature)
    with raises(TypeError):
        radius_law.calculate_radius(100, test_args.order_of_ring,
            test_args.refractive_index_between_lens_plate, test_args.radius_curvature)
...
def test_bad_order_of_ring(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        radius_law.calculate_radius(test_args.wavelength, order_of_ring,
            test_args.refractive_index_between_lens_plate, test_args.radius_curvature)
    with raises(ValueError):
        radius_law.calculate_radius(test_args.wavelength, -1,
            test_args.refractive_index_between_lens_plate, test_args.radius_curvature)
...
def test_bad_refractive_index_between_lens_plate(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        radius_law.calculate_radius(test_args.wavelength, test_args.order_of_ring,
            refractive_index_between_lens_plate, test_args.radius_curvature)
...
def test_bad_radius_curvature(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        radius_law.calculate_radius(test_args.wavelength, test_args.order_of_ring,
            test_args.refractive_index_between_lens_plate, radius_curvature)
    with raises(TypeError):
        radius_law.calculate_radius(test_args.wavelength, test_args.order_of_ring,
            test_args.refractive_index_between_lens_plate, 100)
