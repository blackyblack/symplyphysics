...
from symplyphysics.reorganized.circuits.components.varactors import varactor_junction_p_n_capacity as capacity_law
...
Args = namedtuple("Args", [
    "junction_capacitance_without_bias_voltage", "voltage", "material_parameter",
    "doping_coefficient"
])
...
def test_basic_junction_capacitance(test_args: Args) -> None:
    result = capacity_law.calculate_junction_capacitance(
        test_args.junction_capacitance_without_bias_voltage, test_args.voltage,
        test_args.material_parameter, test_args.doping_coefficient)
...
def test_bad_junction_capacitance_without_bias_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        capacity_law.calculate_junction_capacitance(junction_capacitance_without_bias_voltage,
            test_args.voltage, test_args.material_parameter, test_args.doping_coefficient)
    with raises(TypeError):
        capacity_law.calculate_junction_capacitance(100, test_args.voltage,
            test_args.material_parameter, test_args.doping_coefficient)
...
def test_bad_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        capacity_law.calculate_junction_capacitance(
            test_args.junction_capacitance_without_bias_voltage, voltage,
            test_args.material_parameter, test_args.doping_coefficient)
    with raises(TypeError):
        capacity_law.calculate_junction_capacitance(
            test_args.junction_capacitance_without_bias_voltage, 100, test_args.material_parameter,
            test_args.doping_coefficient)
...
def test_bad_material_parameter(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        capacity_law.calculate_junction_capacitance(
            test_args.junction_capacitance_without_bias_voltage, test_args.voltage,
            material_parameter, test_args.doping_coefficient)
    with raises(TypeError):
        capacity_law.calculate_junction_capacitance(
            test_args.junction_capacitance_without_bias_voltage, test_args.voltage, 100,
            test_args.doping_coefficient)
...
def test_bad_doping_coefficient(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        capacity_law.calculate_junction_capacitance(
            test_args.junction_capacitance_without_bias_voltage, test_args.voltage,
            test_args.material_parameter, doping_coefficient)
