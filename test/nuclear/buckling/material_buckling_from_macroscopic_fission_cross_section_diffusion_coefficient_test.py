...
def test_basic_buckling(test_args):
    result = material_buckling.calculate_buckling(test_args.v, test_args.Sf,
...
def test_bad_macroscopic_cross_section(test_args):
    ...
    with raises(errors.UnitsError):
        material_buckling.calculate_buckling(test_args.v, Sb, test_args.Sa,
                                             test_args.D)
    with raises(TypeError):
        material_buckling.calculate_buckling(test_args.v, 100, test_args.Sa,
                                             test_args.D)
    with raises(errors.UnitsError):
        material_buckling.calculate_buckling(test_args.v, test_args.Sf, Sb,
                                             test_args.D)
    with raises(TypeError):
        material_buckling.calculate_buckling(test_args.v, test_args.Sf, 100,
                                             test_args.D)
...
def test_bad_diffusion_coefficient(test_args):
    ...
    with raises(errors.UnitsError):
        material_buckling.calculate_buckling(test_args.v, test_args.Sf,
                                             test_args.Sa, Db)
    with raises(TypeError):
        material_buckling.calculate_buckling(test_args.v, test_args.Sf,
                                             test_args.Sa, 100)
