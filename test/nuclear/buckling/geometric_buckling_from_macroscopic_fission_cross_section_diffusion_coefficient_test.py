...
def test_basic_buckling(test_args):
    result = buckling.calculate_buckling(test_args.v, test_args.k, test_args.Sf,
                                         test_args.Sa, test_args.D)
    assert SI.get_dimension_system().equivalent_dims(result.dimension,
                                                     1 / units.length**2)
    result_buckling = convert_to(result,
                                 1 / units.meter**2).subs(units.meter,
                                                          1).evalf(2)
...
def test_bad_macroscopic_cross_section(test_args):
    ...
    with raises(errors.UnitsError):
        buckling.calculate_buckling(test_args.v, test_args.k, Sb, test_args.Sa,
                                    test_args.D)
    with raises(TypeError):
        buckling.calculate_buckling(test_args.v, test_args.k, 100, test_args.Sa,
                                    test_args.D)
    with raises(errors.UnitsError):
        buckling.calculate_buckling(test_args.v, test_args.k, test_args.Sf, Sb,
                                    test_args.D)
    with raises(TypeError):
        buckling.calculate_buckling(test_args.v, test_args.k, test_args.Sf, 100,
                                    test_args.D)
...
def test_bad_diffusion_coefficient(test_args):
    ...
    with raises(errors.UnitsError):
        buckling.calculate_buckling(test_args.v, test_args.k, test_args.Sf,
                                    test_args.Sa, Db)
    with raises(TypeError):
        buckling.calculate_buckling(test_args.v, test_args.k, test_args.Sf,
                                    test_args.Sa, 100)
