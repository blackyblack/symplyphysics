...
def test_basic_buckling(test_args):
    result = buckling.calculate_geometric_buckling_squared(
        test_args.k_inf, test_args.k_eff, test_args.L2)
    assert SI.get_dimension_system().equivalent_dims(result.dimension,
                                                     units.length**-2)
    result_buckling = convert_to(result, units.centimeter**-2).subs(
        units.centimeter, 1).evalf(4)
...
def test_bad_diffusion_area(test_args):
    ...
    with raises(errors.UnitsError):
        buckling.calculate_geometric_buckling_squared(test_args.k_inf,
                                                      test_args.k_eff, L2b)
