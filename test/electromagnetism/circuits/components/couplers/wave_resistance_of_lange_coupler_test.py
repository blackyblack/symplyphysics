...
from symplyphysics.electromagnetism.circuits.components.couplers import wave_resistance_of_lange_coupler as resistance_law
...
Args = namedtuple("Args",
    ["wave_resistance_odd_modes", "wave_resistance_even_modes", "number_segments"])
...
def test_bad_resistance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        resistance_law.calculate_wave_resistance(bad_resistance,
            test_args.wave_resistance_even_modes, test_args.number_segments)
    ...
    with raises(errors.UnitsError):
        resistance_law.calculate_wave_resistance(test_args.wave_resistance_odd_modes,
            bad_resistance, test_args.number_segments)
    with raises(TypeError):
