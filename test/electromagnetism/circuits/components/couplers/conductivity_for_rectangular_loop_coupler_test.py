...
from symplyphysics.electromagnetism.circuits.components.couplers import conductivity_for_rectangular_loop_coupler as conductivity_law
...
def test_basic_conductivities(test_args: Args) -> None:
    result = conductivity_law.calculate_conductivities(test_args.transmission_line_conductivity,
...
def test_bad_transmission_line_conductivity(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        conductivity_law.calculate_conductivities(bad_transmission_line_conductivity,
            test_args.ratio_of_power)
    with raises(TypeError):
        conductivity_law.calculate_conductivities(100, test_args.ratio_of_power)


def test_bad_ratio_of_power(test_args: Args) -> None:
    bad_ratio_of_power = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        conductivity_law.calculate_conductivities(test_args.transmission_line_conductivity,
            bad_ratio_of_power)
