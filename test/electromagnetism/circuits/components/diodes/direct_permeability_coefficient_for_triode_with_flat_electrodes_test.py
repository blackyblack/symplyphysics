...
from symplyphysics.electromagnetism.circuits.components.diodes import direct_permeability_coefficient_for_triode_with_flat_electrodes as coefficient_law
...
def test_basic_direct_permeability_coefficient(test_args: Args) -> None:
    result = coefficient_law.calculate_direct_permeability_coefficient(
        test_args.tabular_coefficient_t, test_args.grid_step, test_args.distance_to_grid,
        test_args.tabular_coefficient_d)
...
def test_bad_tabular_coefficient_t(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_direct_permeability_coefficient(tabular_coefficient_t,
            test_args.grid_step, test_args.distance_to_grid, test_args.tabular_coefficient_d)


def test_bad_grid_step(test_args: Args) -> None:
    grid_step = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        coefficient_law.calculate_direct_permeability_coefficient(test_args.tabular_coefficient_t,
            grid_step, test_args.distance_to_grid, test_args.tabular_coefficient_d)
    with raises(TypeError):
        coefficient_law.calculate_direct_permeability_coefficient(test_args.tabular_coefficient_t,
            100, test_args.distance_to_grid, test_args.tabular_coefficient_d)
...
def test_bad_distance_to_grid(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_direct_permeability_coefficient(test_args.tabular_coefficient_t,
            test_args.grid_step, distance_to_grid, test_args.tabular_coefficient_d)
    with raises(TypeError):
        coefficient_law.calculate_direct_permeability_coefficient(test_args.tabular_coefficient_t,
            test_args.grid_step, 100, test_args.tabular_coefficient_d)
...
def test_bad_tabular_coefficient_d(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_direct_permeability_coefficient(test_args.tabular_coefficient_t,
            test_args.grid_step, test_args.distance_to_grid, tabular_coefficient_d)
