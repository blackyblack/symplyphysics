...
from symplyphysics.electromagnetism.circuits.components.diodes import steepness_of_volt_ampere_characteristic_of_vacuum_diode as steepness_law
...
def test_basic_steepness(test_args: Args) -> None:
    result = steepness_law.calculate_steepness(test_args.diode_constant, test_args.voltage)
...
def test_bad_diode_constant(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        steepness_law.calculate_steepness(diode_constant, test_args.voltage)
    with raises(TypeError):
        steepness_law.calculate_steepness(100, test_args.voltage)


def test_bad_voltage(test_args: Args) -> None:
    voltage = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        steepness_law.calculate_steepness(test_args.diode_constant, voltage)
    with raises(TypeError):
        steepness_law.calculate_steepness(test_args.diode_constant, 100)
