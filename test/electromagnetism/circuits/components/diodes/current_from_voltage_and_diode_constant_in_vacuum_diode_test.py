...
from symplyphysics.electromagnetism.circuits.components.diodes import current_from_voltage_and_diode_constant_in_vacuum_diode as current_law
...
def test_basic_current(test_args: Args) -> None:
    result = current_law.calculate_current(test_args.diode_constant, test_args.voltage)
...
def test_bad_diode_constant(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_law.calculate_current(diode_constant, test_args.voltage)
    with raises(TypeError):
        current_law.calculate_current(100, test_args.voltage)


def test_bad_voltage(test_args: Args) -> None:
    voltage = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        current_law.calculate_current(test_args.diode_constant, voltage)
    with raises(TypeError):
        current_law.calculate_current(test_args.diode_constant, 100)
