...
from symplyphysics.electromagnetism.circuits.components.diodes import internal_resistance_of_vacuum_diode as resistance_law
...
def test_args_fixture() -> Args:
    ...
    return Args(diode_constant=diode_constant, voltage=voltage)


def test_basic_internal_resistance(test_args: Args) -> None:
    result = resistance_law.calculate_internal_resistance(test_args.diode_constant,
        test_args.voltage)
...
def test_bad_diode_constant(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        resistance_law.calculate_internal_resistance(diode_constant, test_args.voltage)
    with raises(TypeError):
        resistance_law.calculate_internal_resistance(100, test_args.voltage)


def test_bad_voltage(test_args: Args) -> None:
    voltage = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        resistance_law.calculate_internal_resistance(test_args.diode_constant, voltage)
    with raises(TypeError):
        resistance_law.calculate_internal_resistance(test_args.diode_constant, 100)
