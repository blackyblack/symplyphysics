...
from symplyphysics.electromagnetism.circuits.components.cavity_resonators import quality_factor_of_empty_rectangular_resonator_for_transverse_electric_waves_with_indices_101 as factor_law
...
Args = namedtuple("Args", [
    "angular_frequency", "absolute_permeability", "surface_resistance", "resonator_width",
...
def test_basic_quality_factor(test_args: Args) -> None:
    result = factor_law.calculate_quality_factor(test_args.angular_frequency,
        test_args.absolute_permeability, test_args.surface_resistance,
...
def test_bad_angular_frequency(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        factor_law.calculate_quality_factor(angular_frequency, test_args.absolute_permeability,
            test_args.surface_resistance,
            (test_args.resonator_width, test_args.resonator_height, test_args.resonator_length))
    with raises(TypeError):
        factor_law.calculate_quality_factor(100, test_args.absolute_permeability,
            test_args.surface_resistance,
            (test_args.resonator_width, test_args.resonator_height, test_args.resonator_length))


def test_bad_absolute_permeability(test_args: Args) -> None:
    absolute_permeability = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        factor_law.calculate_quality_factor(test_args.angular_frequency, absolute_permeability,
            test_args.surface_resistance,
            (test_args.resonator_width, test_args.resonator_height, test_args.resonator_length))
    with raises(TypeError):
        factor_law.calculate_quality_factor(test_args.angular_frequency, 100,
            test_args.surface_resistance,
            (test_args.resonator_width, test_args.resonator_height, test_args.resonator_length))
...
def test_bad_surface_resistance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        factor_law.calculate_quality_factor(test_args.angular_frequency,
            test_args.absolute_permeability, bad_surface_resistance,
            (test_args.resonator_width, test_args.resonator_height, test_args.resonator_length))
    with raises(TypeError):
        factor_law.calculate_quality_factor(test_args.angular_frequency,
            test_args.absolute_permeability, 100,
            (test_args.resonator_width, test_args.resonator_height, test_args.resonator_length))
