...
from symplyphysics.electromagnetism.circuits.components.filters import filter_order_from_distortion_and_frequencies as order_law
...
def test_basic_order(test_args: Args) -> None:
    result = order_law.calculate_order(test_args.filter_function, test_args.band_stop_frequency,
        test_args.bandwidth_distortion, test_args.band_stop_distortion)
...
def test_bad_band_stop_frequency(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        order_law.calculate_order(test_args.filter_function, band_stop_frequency,
            test_args.bandwidth_distortion, test_args.band_stop_distortion)
    with raises(TypeError):
        order_law.calculate_order(test_args.filter_function, 100, test_args.bandwidth_distortion,
            test_args.band_stop_distortion)


def test_bad_distortions(test_args: Args) -> None:
    bad_distortion = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        order_law.calculate_order(test_args.filter_function, test_args.band_stop_frequency,
            bad_distortion, test_args.band_stop_distortion)
    with raises(errors.UnitsError):
        order_law.calculate_order(test_args.filter_function, test_args.band_stop_frequency,
            test_args.bandwidth_distortion, bad_distortion)
