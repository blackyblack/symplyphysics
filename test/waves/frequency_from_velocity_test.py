...
def test_args():
    ...
    Args = namedtuple("Args", [
        "sound_velocity", "train_velocity", "bike_velocity", "zero_velocity",
        "horn_frequency"
    ])
...
def test_basic_frequency(test_args):
    result_1 = doppler_law.calculate_observed_frequency(
        test_args.horn_frequency, test_args.sound_velocity,
        test_args.train_velocity, test_args.zero_velocity)
    result_2 = doppler_law.calculate_observed_frequency(
        test_args.horn_frequency, test_args.sound_velocity,
        test_args.zero_velocity, test_args.bike_velocity)
    assert SI.get_dimension_system().equivalent_dims(result_1.dimension,
                                                     units.frequency)
    assert SI.get_dimension_system().equivalent_dims(result_2.dimension,
                                                     units.frequency)
    result_freq_1 = convert_to(result_1, units.hertz).subs(units.hertz,
                                                           1).evalf(4)
    assert result_freq_1 == approx(2015, 0.1)
    result_freq_2 = convert_to(result_2, units.hertz).subs(units.hertz,
                                                           1).evalf(4)
...
def test_bad_velocity(test_args):
    ...
    with raises(errors.UnitsError):
        doppler_law.calculate_observed_frequency(test_args.horn_frequency, vb,
                                                 test_args.train_velocity,
                                                 test_args.bike_velocity)
    with raises(TypeError):
        doppler_law.calculate_observed_frequency(test_args.horn_frequency, 100,
                                                 test_args.train_velocity,
                                                 test_args.bike_velocity)
    with raises(errors.UnitsError):
        doppler_law.calculate_observed_frequency(test_args.horn_frequency,
                                                 test_args.sound_velocity, vb,
                                                 test_args.bike_velocity)
    with raises(TypeError):
        doppler_law.calculate_observed_frequency(test_args.horn_frequency,
                                                 test_args.sound_velocity, 100,
                                                 test_args.bike_velocity)
    with raises(errors.UnitsError):
        doppler_law.calculate_observed_frequency(test_args.horn_frequency,
                                                 test_args.sound_velocity,
