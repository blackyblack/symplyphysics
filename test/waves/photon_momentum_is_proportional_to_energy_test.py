...
from symplyphysics.laws.waves import photon_momentum_is_proportional_to_energy as photon_momentum_law

# Description
## Assert we have ultraviolet radiation with e of 3e16 Hz.
...
def test_args_fixture() -> Args:
    f = Quantity(3e16 * units.hertz)
    e = Quantity(units.planck * f)
    return Args(e=e)


def test_basic_momentum(test_args: Args) -> None:
    result = photon_momentum_law.calculate_momentum(test_args.e)
...
def test_bad_energy() -> None:
    ...
    with raises(errors.UnitsError):
        photon_momentum_law.calculate_momentum(eb)
