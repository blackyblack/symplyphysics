...
def test_basic_charge(test_args):
    result = current_def.calculate_charge(
        test_args.I0, test_args.I1, test_args.t)
    assert SI.get_dimension_system().equivalent_dims(
        result.dimension, units.charge)

    result_charge = convert_to(result, current_def.definition_dimension_SI).subs({
        units.coulomb: 1}).evalf(2)
    assert result_charge == approx(2.75, 0.01)
