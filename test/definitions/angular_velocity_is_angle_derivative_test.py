...
def test_basic_velocity(test_args):
    result = angular_velocity_def.calculate_angular_velocity(
        test_args.a0, test_args.a1, test_args.t)
    assert SI.get_dimension_system().equivalent_dims(result.dimension,
                                                     1 / units.time)
    result_velocity = convert_to(
        result, angular_velocity_def.definition_units_SI).subs({
            units.radian: 1,
            units.second: 1
        }).evalf(2)
...
def test_velocity_with_number(test_args):
    angular_velocity_def.calculate_angular_velocity(100, test_args.a1,
                                                    test_args.t)
    angular_velocity_def.calculate_angular_velocity(test_args.a0, 100,
                                                    test_args.t)
...
def test_velocity_with_bad_angle(test_args):
    ...
    with raises(errors.UnitsError):
        angular_velocity_def.calculate_angular_velocity(ab, test_args.a1,
                                                        test_args.t)
    with raises(errors.UnitsError):
        angular_velocity_def.calculate_angular_velocity(test_args.a0, ab,
                                                        test_args.t)
...
def test_velocity_with_bad_time(test_args):
    ...
    with raises(errors.UnitsError):
        angular_velocity_def.calculate_angular_velocity(test_args.a0,
                                                        test_args.a1, tb)
