...
def test_args():
    ...
    field = VectorField(
        0, lambda point: -1 * force_unit * radius_unit**2 / point.y**2, 0, C)
...
def test_basic_circulation(test_args):
    field = VectorField(lambda point: point.y, 0,
                        lambda point: point.x + point.z, test_args.C)
...
def test_two_parameters_circulation(test_args):
    field = VectorField(lambda point: point.y, lambda point: -point.x, 0,
                        test_args.C)
    # circle function is: x**2 + y**2 = 9
    # parametrize by circulation_def.parameter
    circle = [
        3 * cos(circulation_def.parameter), 3 * sin(circulation_def.parameter)
    ]
    result_expr = circulation_def.calculate_circulation(field, circle, 0,
                                                        2 * pi)
    ...
    circle_implicit_up = [
        ...
        sqrt(9 - circulation_def.parameter**2)
    ]
    result_expr_up = circulation_def.calculate_circulation(
        field, circle_implicit_up, 3, -3)
    circle_implicit_down = [
        circulation_def.parameter, -sqrt(9 - circulation_def.parameter**2)
    ]
    result_expr_down = circulation_def.calculate_circulation(
        field, circle_implicit_down, -3, 3)
    assert (result_expr_up + result_expr_down).evalf(4) == approx(
        -18 * pi, 0.001)
...
def test_orthogonal_movement_circulation(test_args):
    ...
    result_expr = circulation_def.calculate_circulation(field,
...
def test_force_circulation(test_args):
    ...
    result_expr = circulation_def.calculate_circulation(
        test_args.field, trajectory, 1 * test_args.radius_unit,
        2 * test_args.radius_unit)
    result = expr_to_quantity(result_expr)
    assert SI.get_dimension_system().equivalent_dims(result.dimension,
                                                     units.energy)
    result_work = convert_to(result, units.joule).subs({
        units.joule: 1
    }).evalf(2)
...
def test_force_circulation_horizontal(test_args):
    ...
    trajectory_horizontal = [
        circulation_def.parameter, 5 * test_args.radius_unit
    ]
    result_expr = circulation_def.calculate_circulation(
        test_args.field, trajectory_horizontal, 1 * test_args.radius_unit,
        2 * test_args.radius_unit)
    assert result_expr == 0
...
def test_force_circulation_horizontal_up(test_args):
    ...
    result_expr = circulation_def.calculate_circulation(
        test_args.field, trajectory_vertical, 1 * test_args.radius_unit,
        2 * test_args.radius_unit)
    result = expr_to_quantity(result_expr)
    result_work = convert_to(result, units.joule).subs({
        units.joule: 1
...
def test_force_circulation_horizontal_down(test_args):
    ...
    result_expr = circulation_def.calculate_circulation(
        test_args.field, trajectory_vertical, 2 * test_args.radius_unit,
        1 * test_args.radius_unit)
    result = expr_to_quantity(result_expr)
    result_work = convert_to(result, units.joule).subs({
        units.joule: 1
    }).evalf(2)
