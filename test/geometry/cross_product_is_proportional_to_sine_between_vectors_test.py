...
def test_law(test_args):
    result = sine_law.calculate_sine_between_vectors(test_args.tau_norm, test_args.r_norm, test_args.F_norm)
    assert_equal(result, 0.683)


def test_law_perpendicular(test_args):
    tau_perpendicular_norm = test_args.r_norm * test_args.F_norm
    result = sine_law.calculate_sine_between_vectors(tau_perpendicular_norm, test_args.r_norm, test_args.F_norm)
    assert_equal(result, 1.0)


def test_law_parallel(test_args):
    tau_parallel_norm = 0.0
    result = sine_law.calculate_sine_between_vectors(tau_parallel_norm, test_args.r_norm, test_args.F_norm)
