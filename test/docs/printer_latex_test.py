...
def test_mul_fraction_and_sin() -> None:
    ...
    time = symbols.time
    with evaluate(False):
        expr = (driving_force_amplitude / (2 * mass * natural_angular_frequency)) * time * sin(natural_angular_frequency * time)
...
def test_fraction_mul_inner_fraction() -> None:
    ...
    with evaluate(False):
        expr = first_charge * second_charge / (4 * pi * quantities.vacuum_permittivity) / distance**2
...
def test_fraction_mul_fraction() -> None:
    ...
    distance = symbols.distance
    with evaluate(False):
        expr = 1 / (2 * pi * quantities.vacuum_permittivity) * (electric_dipole_moment / distance**3)
