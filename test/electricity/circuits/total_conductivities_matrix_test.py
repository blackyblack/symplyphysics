...
def test_args_fixture() -> Args:
    ...
    conductivities = ((Quantity(100 * units.siemens), Quantity(50 * units.siemens)), (Quantity(50 * units.siemens), Quantity(1000 * units.siemens)))
...
def test_basic_voltages(test_args: Args) -> None:
    result = conductivity_matrix_law.calculate_voltages(test_args.input_current, test_args.output_current, test_args.conductivities)
...
def test_bad_voltages(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        conductivity_matrix_law.calculate_voltages(bad_voltage, test_args.output_current, test_args.conductivities)
    with raises(TypeError):
        conductivity_matrix_law.calculate_voltages(100, test_args.output_current, test_args.conductivities)
    with raises(errors.UnitsError):
        conductivity_matrix_law.calculate_voltages(test_args.input_current, bad_voltage, test_args.conductivities)
    with raises(TypeError):
        conductivity_matrix_law.calculate_voltages(test_args.input_current, 100, test_args.conductivities)
