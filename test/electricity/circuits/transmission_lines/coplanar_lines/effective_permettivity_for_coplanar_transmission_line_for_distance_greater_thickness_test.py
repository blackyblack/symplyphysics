...
from symplyphysics.laws.electricity.circuits.transmission_lines.coplanar_lines import effective_permettivity_for_coplanar_transmission_line_for_distance_greater_thickness as permittivity_law
...
Args = namedtuple("Args", [
    "relative_permittivity", "distance_between_electrodes", "thickness_of_substrate",
    "central_electrode_width"
])
...
def test_basic_effective_permittivity(test_args: Args) -> None:
    result = permittivity_law.calculate_effective_permittivity(
        test_args.relative_permittivity, test_args.distance_between_electrodes,
        test_args.thickness_of_substrate, test_args.central_electrode_width)
...
def test_bad_distance_between_electrodes(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
            bad_distance_between_electrodes, test_args.thickness_of_substrate,
            test_args.central_electrode_width)
...
def test_bad_thickness_of_substrate(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
            test_args.distance_between_electrodes, bad_thickness_of_substrate,
            test_args.central_electrode_width)
    with raises(TypeError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
    ...
    with raises(ValueError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
            test_args.thickness_of_substrate, test_args.distance_between_electrodes,
            test_args.central_electrode_width)
...
def test_bad_central_electrode_width(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
            test_args.distance_between_electrodes, test_args.thickness_of_substrate,
            bad_central_electrode_width)
    with raises(TypeError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
            test_args.distance_between_electrodes, test_args.thickness_of_substrate, 100)
    with raises(ValueError):
        permittivity_law.calculate_effective_permittivity(test_args.relative_permittivity,
            test_args.central_electrode_width, test_args.thickness_of_substrate,
            test_args.central_electrode_width)
