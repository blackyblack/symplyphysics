...
from symplyphysics.laws.electricity.circuits.transmission_lines.coplanar_lines import wave_resistance_of_coplanar_line_second as resistance_law

## The width of the central electrode of the coplanar line is 9 millimeter, and the distance between electrodes is 10 millimeter.
## The effective permittivity is 4. Then the wave resistance is 34.19 ohm.
...
def test_args_fixture() -> Args:
    ...
    central_electrode_width = Quantity(9 * units.millimeter)
...
def test_basic_wave_resistance(test_args: Args) -> None:
    ...
    assert_equal(result, 34.19 * units.ohm)
...
def test_bad_central_electrode_width(test_args: Args) -> None:
    ...
    with raises(ValueError):
        resistance_law.calculate_wave_resistance(test_args.effective_permittivity,
            test_args.central_electrode_width, test_args.distance_between_electrodes)
    central_electrode_width = Quantity(1 * units.millimeter)
    with raises(ValueError):
        resistance_law.calculate_wave_resistance(test_args.effective_permittivity,
            test_args.distance_between_electrodes, central_electrode_width)
