...
from symplyphysics.laws.electricity.circuits.transmission_lines.microstrip_lines import attenuation_coefficient_in_metal_of_microstrip_line_for_width_less_thickness_first as coefficient_law

## The surface resistance of the metal strip is 2.576 milliohm. The wave resistance of the line is 70 ohm. The thickness
## of the substrate and the metal strip is 32 millimeter and 50 micrometer, respectively. The width of the strip is 6 millimeter,
## and the effective width of the line is 5 millimeter. Then the losses in the metal strip will be equal to 0.0413 [1 / meter].
...
def test_args_fixture() -> Args:
    ...
    effective_width = Quantity(5 * units.millimeter)
    strip_thickness = Quantity(50 * units.micrometer)
    width = Quantity(6 * units.millimeter)
...
def test_basic_attenuation_coefficient(test_args: Args) -> None:
    ...
    assert_equal(result, 0.0413 * (1 / units.meter))
