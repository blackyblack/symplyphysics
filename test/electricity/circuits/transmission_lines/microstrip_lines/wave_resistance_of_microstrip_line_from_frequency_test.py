...
from symplyphysics.laws.electricity.circuits.transmission_lines.microstrip_lines import wave_resistance_of_microstrip_line_from_frequency as resistance_law
...
Args = namedtuple("Args", [
    "wave_resistance_without_frequency", "effective_permittivity",
...
def test_basic_wave_resistance(test_args: Args) -> None:
    result = resistance_law.calculate_wave_resistance(
        test_args.wave_resistance_without_frequency, test_args.effective_permittivity,
...
def test_bad_wave_resistance_without_frequency(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        resistance_law.calculate_wave_resistance(bad_wave_resistance_without_frequency,
...
def test_bad_effective_permittivities(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        resistance_law.calculate_wave_resistance(test_args.wave_resistance_without_frequency,
            bad_effective_permittivity, test_args.effective_permittivity_without_frequency)
    with raises(errors.UnitsError):
        resistance_law.calculate_wave_resistance(test_args.wave_resistance_without_frequency,
            test_args.effective_permittivity, bad_effective_permittivity)
