...
Args = namedtuple("Args", ["distance_to_anode", "distance_to_grid", "anode_voltage", "voltage_triode_gain", "grid_voltage"])
...
def test_basic_voltage_of_equivalent_diode(test_args: Args) -> None:
    result = voltage_law.calculate_voltage_of_equivalent_diode(
        test_args.distance_to_anode, test_args.distance_to_grid, test_args.anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_distance_to_anode(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        voltage_law.calculate_voltage_of_equivalent_diode(distance_to_anode,
            test_args.distance_to_grid, test_args.anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
    with raises(TypeError):
        voltage_law.calculate_voltage_of_equivalent_diode(100,
            test_args.distance_to_grid, test_args.anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_distance_to_grid(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        voltage_law.calculate_voltage_of_equivalent_diode(test_args.distance_to_anode,
            distance_to_grid, test_args.anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
    with raises(TypeError):
        voltage_law.calculate_voltage_of_equivalent_diode(test_args.distance_to_anode,
            100, test_args.anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_anode_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        voltage_law.calculate_voltage_of_equivalent_diode(
            test_args.distance_to_anode, test_args.distance_to_grid, anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
    with raises(TypeError):
        voltage_law.calculate_voltage_of_equivalent_diode(
            test_args.distance_to_anode, test_args.distance_to_grid, 100, test_args.voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_voltage_triode_gain(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        voltage_law.calculate_voltage_of_equivalent_diode(
            test_args.distance_to_anode, test_args.distance_to_grid, test_args.anode_voltage, voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_grid_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        voltage_law.calculate_voltage_of_equivalent_diode(
            test_args.distance_to_anode, test_args.distance_to_grid, test_args.anode_voltage, test_args.voltage_triode_gain, grid_voltage)
    with raises(TypeError):
        voltage_law.calculate_voltage_of_equivalent_diode(
            test_args.distance_to_anode, test_args.distance_to_grid, test_args.anode_voltage, test_args.voltage_triode_gain, 100)
