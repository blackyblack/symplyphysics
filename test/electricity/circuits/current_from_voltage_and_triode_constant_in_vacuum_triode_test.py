...
Args = namedtuple("Args", ["triode_constant", "anode_voltage", "voltage_triode_gain", "grid_voltage"])
...
def test_basic_current(test_args: Args) -> None:
    result = current_law.calculate_current(
        test_args.triode_constant, test_args.anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_triode_constant(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_law.calculate_current(triode_constant,
...
def test_bad_anode_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_law.calculate_current(
            test_args.triode_constant, anode_voltage, test_args.voltage_triode_gain, test_args.grid_voltage)
    with raises(TypeError):
        current_law.calculate_current(
            test_args.triode_constant, 100, test_args.voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_voltage_triode_gain(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_law.calculate_current(
            test_args.triode_constant, test_args.anode_voltage, voltage_triode_gain, test_args.grid_voltage)
...
def test_bad_grid_voltage(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        current_law.calculate_current(
            test_args.triode_constant, test_args.anode_voltage, test_args.voltage_triode_gain, grid_voltage)
    with raises(TypeError):
        current_law.calculate_current(
            test_args.triode_constant, test_args.anode_voltage, test_args.voltage_triode_gain, 100)
