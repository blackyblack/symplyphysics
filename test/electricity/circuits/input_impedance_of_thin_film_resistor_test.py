...
Args = namedtuple("Args", ["resistance", "frequency", "capacitance",])
...
def test_basic_input_impedance(test_args: Args) -> None:
    result = impedance_law.calculate_input_impedance(test_args.resistance, test_args.frequency, test_args.capacitance)
...
def test_bad_resistance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(resistance, test_args.frequency, test_args.capacitance)
...
def test_bad_frequency(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(test_args.resistance, frequency, test_args.capacitance)
    with raises(TypeError):
...
def test_bad_capacitance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(test_args.resistance, test_args.frequency, capacitance)
    with raises(TypeError):
