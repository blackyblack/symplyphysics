...
from symplyphysics.laws.electricity.circuits.waveguides import attenuation_coefficient_in_dielectric_in_rectangular_waveguide as coefficient_law
...
Args = namedtuple("Args", [
    "resistance_of_waveguide", "resistance_of_medium", "wavelength", "tangent_dielectric_loss_angle"
])
...
def test_args_fixture() -> Args:
    ...
    return Args(resistance_of_waveguide=resistance_of_waveguide,
...
def test_basic_attenuation_coefficient(test_args: Args) -> None:
    result = coefficient_law.calculate_attenuation_coefficient(
        test_args.resistance_of_waveguide, test_args.resistance_of_medium, test_args.wavelength,
...
def test_bad_resistance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_attenuation_coefficient(bad_resistance,
            test_args.resistance_of_medium, test_args.wavelength,
            test_args.tangent_dielectric_loss_angle)
    with raises(TypeError):
        coefficient_law.calculate_attenuation_coefficient(100, test_args.resistance_of_medium,
            test_args.wavelength, test_args.tangent_dielectric_loss_angle)
    with raises(errors.UnitsError):
        coefficient_law.calculate_attenuation_coefficient(test_args.resistance_of_waveguide,
            bad_resistance, test_args.wavelength, test_args.tangent_dielectric_loss_angle)
    with raises(TypeError):
        coefficient_law.calculate_attenuation_coefficient(test_args.resistance_of_waveguide, 100,
            test_args.wavelength, test_args.tangent_dielectric_loss_angle)
...
def test_bad_wavelength(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_attenuation_coefficient(test_args.resistance_of_waveguide,
            test_args.resistance_of_medium, bad_wavelength, test_args.tangent_dielectric_loss_angle)
    with raises(TypeError):
        coefficient_law.calculate_attenuation_coefficient(test_args.resistance_of_waveguide,
            test_args.resistance_of_medium, 100, test_args.tangent_dielectric_loss_angle)
...
def test_bad_tangent_dielectric_loss_angle(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        coefficient_law.calculate_attenuation_coefficient(test_args.resistance_of_waveguide,
            test_args.resistance_of_medium, test_args.wavelength, tangent_dielectric_loss_angle)
