...
from symplyphysics.laws.electricity.circuits import quality_factor_of_filled_rectangular_resonator as factor_law
...
def test_args_fixture() -> Args:
    ...
    return Args(empty_resonator_quality_factor=empty_resonator_quality_factor, tangent_dielectric_loss_angle=tangent_dielectric_loss_angle)


def test_basic_quality_factor(test_args: Args) -> None:
    result = factor_law.calculate_quality_factor(test_args.empty_resonator_quality_factor, test_args.tangent_dielectric_loss_angle)
...
def test_bad_empty_resonator_quality_factor(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        factor_law.calculate_quality_factor(empty_resonator_quality_factor, test_args.tangent_dielectric_loss_angle)


def test_bad_tangent_dielectric_loss_angle(test_args: Args) -> None:
    tangent_dielectric_loss_angle = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        factor_law.calculate_quality_factor(test_args.empty_resonator_quality_factor, tangent_dielectric_loss_angle)
