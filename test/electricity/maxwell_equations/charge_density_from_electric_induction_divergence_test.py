...
def test_basic_bulk_density_of_electric_charge(test_args: Args) -> None:
    result = gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (test_args.x, test_args.y, test_args.z))
...
def test_bad_coordinates(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (bad_coordinate, test_args.y, test_args.z))
    with raises(errors.UnitsError):
        gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (test_args.x, bad_coordinate, test_args.z))
    with raises(errors.UnitsError):
        gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (test_args.x, test_args.y, bad_coordinate))
    with raises(TypeError):
        gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (100, test_args.y, test_args.z))
    with raises(TypeError):
        gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (test_args.x, 100, test_args.z))
    with raises(TypeError):
        gauss_law.calculate_charge_volumetric_density_at_point(test_args.electrical_induction, (test_args.x, test_args.y, 100))
