...
Args = namedtuple("Args", ["own_quality_factor", "external_circuit_quality_factor"])


@fixture(name="test_args")
def test_args_fixture() -> Args:
    own_quality_factor = 8000
    external_circuit_quality_factor = 10000

    return Args(own_quality_factor=own_quality_factor, external_circuit_quality_factor=external_circuit_quality_factor)


def test_basic_coupling_parameter(test_args: Args) -> None:
    result = parameter_law.calculate_coupling_parameter(test_args.own_quality_factor,
...
def test_bad_quality_factors(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        parameter_law.calculate_coupling_parameter(bad_quality_factor,
            test_args.external_circuit_quality_factor)
    with raises(errors.UnitsError):
        parameter_law.calculate_coupling_parameter(test_args.own_quality_factor,
            bad_quality_factor)
