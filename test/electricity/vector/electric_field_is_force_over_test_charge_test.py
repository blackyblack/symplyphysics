...
from symplyphysics.laws.electricity.vector import electric_field_is_force_over_test_charge as electric_field_definition
...
def test_basic_electric_field_definition(test_args):
    result = electric_field_definition.calculate_electric_field(test_args.F, test_args.q0)
...
def test_basic_electrostatic_force_law(test_args):
    result = electric_field_definition.calculate_electrostatic_force(test_args.E, test_args.q0)
    assert len(result.components) == 3
...
def test_bad_force_in_electric_field(test_args):
    ...
    with raises(errors.UnitsError):
        electric_field_definition.calculate_electric_field(Fb, test_args.q0)
    with raises(TypeError):
        electric_field_definition.calculate_electric_field(100, test_args.q0)


def test_bad_charge_in_electric_field(test_args):
    q0b = Quantity(1 * units.second)
    with raises(errors.UnitsError):
        electric_field_definition.calculate_electric_field(test_args.F, q0b)
    with raises(TypeError):
        electric_field_definition.calculate_electric_field(test_args.F, 100)


def test_bad_electric_field_in_electrostatic_force(test_args):
    Eb = Quantity(1 * units.second)
    with raises(errors.UnitsError):
        electric_field_definition.calculate_electrostatic_force(Eb, test_args.q0)
    with raises(TypeError):
        electric_field_definition.calculate_electrostatic_force(100, test_args.q0)


def test_bad_charge_in_electrostatic_force(test_args):
    q0b = Quantity(1 * units.second)
    with raises(errors.UnitsError):
        electric_field_definition.calculate_electrostatic_force(test_args.E, q0b)
    with raises(TypeError):
        electric_field_definition.calculate_electrostatic_force(test_args.E, 100)
