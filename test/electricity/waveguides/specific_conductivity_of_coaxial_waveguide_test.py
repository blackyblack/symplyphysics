...
Args = namedtuple("Args", ["angular_frequency", "specific_capacitance", "tangent_dielectric_loss_angle"])
...
def test_args_fixture() -> Args:
    ...
    return Args(angular_frequency=angular_frequency, specific_capacitance=specific_capacitance, tangent_dielectric_loss_angle=tangent_dielectric_loss_angle)


def test_basic_specific_conductivity(test_args: Args) -> None:
    result = conductivity_law.calculate_specific_conductivity(test_args.angular_frequency, test_args.specific_capacitance,
...
def test_bad_angular_frequency(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        conductivity_law.calculate_specific_conductivity(bad_angular_frequency, test_args.specific_capacitance, test_args.tangent_dielectric_loss_angle)


def test_bad_tangent_dielectric_loss_angle(test_args: Args) -> None:
    tangent_dielectric_loss_angle = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        conductivity_law.calculate_specific_conductivity(test_args.angular_frequency, test_args.specific_capacitance, tangent_dielectric_loss_angle)
