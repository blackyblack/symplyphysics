...
Args = namedtuple("Args", ["characteristic_resistance", "load_resistance", "constant_propagation", "line_length", "loss_factor"])
...
def test_basic_input_impedance(test_args: Args) -> None:
    result = impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
        test_args.load_resistance, test_args.constant_propagation, test_args.line_length, test_args.loss_factor)
...
def test_bad_load_resistance(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
            bad_load_resistance, test_args.constant_propagation, test_args.line_length, test_args.loss_factor)
    with raises(TypeError):
...
def test_bad_constant_propagation(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
            test_args.load_resistance, constant_propagation, test_args.line_length, test_args.loss_factor)
    with raises(TypeError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
...
def test_bad_line_length(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
            test_args.load_resistance, test_args.constant_propagation, line_length, test_args.loss_factor)
    with raises(TypeError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
...
def test_bad_loss_factor(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
            test_args.load_resistance, test_args.constant_propagation, test_args.line_length, loss_factor)
    with raises(TypeError):
        impedance_law.calculate_input_impedance(test_args.characteristic_resistance,
