...
def test_bad_velocity(test_args):
    ...
    with raises(errors.UnitsError):
        reynolds_number.calculate_reynolds_number(
            test_args.d, test_args.rho, bv, test_args.mu)
...
def test_bad_diameter(test_args):
    ...
    with raises(errors.UnitsError):
        reynolds_number.calculate_reynolds_number(
            bd, test_args.rho, test_args.v, test_args.mu)
    with raises(TypeError):
        reynolds_number.calculate_reynolds_number(
...
def test_bad_density(test_args):
    ...
    with raises(errors.UnitsError):
        reynolds_number.calculate_reynolds_number(
            test_args.d, bd, test_args.v, test_args.mu)
    with raises(TypeError):
        reynolds_number.calculate_reynolds_number(
            test_args.d, 10, test_args.v, test_args.mu)
...
def test_bad_dynamic_viscosity(test_args):
    ...
    with raises(errors.UnitsError):
        reynolds_number.calculate_reynolds_number(
            test_args.d, test_args.rho, test_args.v, bd)
    with raises(TypeError):
        reynolds_number.calculate_reynolds_number(
