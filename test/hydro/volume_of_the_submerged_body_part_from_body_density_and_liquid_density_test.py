...
def test_basic_volume(test_args):
    result = submerged.calculate_volume(test_args.body_volume, test_args.body_density, test_args.liquid_density)
...
def test_bad_density(test_args):
    ...
    with raises(errors.UnitsError):
        submerged.calculate_volume(test_args.body_volume, bad_density, test_args.liquid_density)
    with raises(errors.UnitsError):
        submerged.calculate_volume(test_args.body_volume, test_args.body_density, bad_density)
    with raises(TypeError):
        submerged.calculate_volume(test_args.body_volume, 100, test_args.liquid_density)
    with raises(TypeError):
        submerged.calculate_volume(test_args.body_volume, test_args.body_density, 100)


def test_bad_volume(test_args):
    bad_volume = Quantity(1 * units.coulomb)
    with raises(errors.UnitsError):
        submerged.calculate_volume(bad_volume, test_args.body_density, test_args.liquid_density)
    with raises(TypeError):
        submerged.calculate_volume(100, test_args.body_density, test_args.liquid_density)
