...
from symplyphysics.laws.kinematic import range_of_flight_of_a_body_thrown_at_an_angle_to_horizon as range_law
...
def test_bad_acceleration(test_args):
    ...
    with raises(errors.UnitsError):
        range_law.calculate_range(test_args.initial_velocity, test_args.angle,
            acceleration)
    with raises(TypeError):
        range_law.calculate_range(test_args.initial_velocity, test_args.angle, 100)
