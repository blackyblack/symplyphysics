...
def test_law(test_args: Args) -> None:
    result = critical_damping.calculate_displacement(
        test_args.x0, test_args.v0, test_args.omega, test_args.t
    )
...
def test_bad_initial_position(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        critical_damping.calculate_displacement(
            xb, test_args.v0, test_args.omega, test_args.t
        )
    with raises(TypeError):
        critical_damping.calculate_displacement(
            100, test_args.v0, test_args.omega, test_args.t
        )
...
def test_bad_initial_velocity(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        critical_damping.calculate_displacement(
            test_args.x0, vb, test_args.omega, test_args.t
        )
    with raises(TypeError):
        critical_damping.calculate_displacement(
            test_args.x0, 100, test_args.omega, test_args.t
        )
...
def test_bad_angular_frequency(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        critical_damping.calculate_displacement(
            test_args.x0, test_args.v0, omega_bad, test_args.t
        )
    with raises(TypeError):
        critical_damping.calculate_displacement(
            test_args.x0, test_args.v0, 100, test_args.t
        )
...
def test_bad_time(test_args: Args) -> None:
    ...
    with raises(errors.UnitsError):
        critical_damping.calculate_displacement(
            test_args.x0, test_args.v0, test_args.omega, tb
        )
    with raises(TypeError):
        critical_damping.calculate_displacement(
            test_args.x0, test_args.v0, test_args.omega, 100
        )
