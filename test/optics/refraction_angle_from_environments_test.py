...
def test_args():
    ...
    Args = namedtuple(
        "Args", ["incedence_media", "incedence_angle", "refractive_media"])
...
def test_basic_angle(test_args):
    result = refraction_law.calculate_refraction_angle(
        test_args.incedence_angle, test_args.incedence_media,
    ...
    result_angle = convert_to(result.scale_factor,
                              units.degree).subs(units.degree, 1).evalf(4)
...
def test_bad_angle(test_args):
    ...
    with raises(AssertionError):
        refraction_law.calculate_refraction_angle(100,
                                                  test_args.incedence_media,
                                                  test_args.refractive_media)
    with raises(AssertionError):
        refraction_law.calculate_refraction_angle(-100,
                                                  test_args.incedence_media,
