...
# Description
## Let the wavelength be 500 nanometer, the order of interference is 9/2,
## radius of curvature is 16 centimeter and the refractive index is 1.
## Then the radius of Newtons's ring will be 0.6 millimeter.
## https://moodle.yspu.org/pluginfile.php/2661/mod_scorm/content/13/node15.html


@fixture(name="test_args")
def test_args_fixture():
    wavelength = Quantity(500 * units.nanometer)
    order_interference = 9/2
    radius_curvature = Quantity(16 * units.centimeter)
    refractive_index = 1
...
def test_basic_radius(test_args):
    ...
    assert result == approx(0.6, rel=0.1)
...
def test_bad_order_interference(test_args):
    ...
    with raises(ValueError):
        radius_law.calculate_radius(test_args.wavelength, 0.3, test_args.refractive_index, test_args.radius_curvature)
    with raises(ValueError):
