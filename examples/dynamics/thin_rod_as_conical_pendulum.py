...
angular_velocity_vector = CoordinateVector([0, 0, angular_velocity], CARTESIAN)
...
element_position_vector = CoordinateVector([
    0,
...
force_from_acceleration_expr = solve_for_vector(force_law.law, force_law.force)
...
gravity_force_acting_on_element = force_from_acceleration_expr.subs({
    force_law.acceleration: free_fall_acceleration_vector,
    force_law.mass: element_mass_expr,
})

gravity_torque_acting_on_element = solve_for_vector(
    torque_def.law,
    torque_def.torque,
).subs({
    torque_def.force: gravity_force_acting_on_element,
    torque_def.position_vector: element_position_vector,
})
...
element_centripetal_acceleration_vector = centripetal_law.law.rhs.subs({
    centripetal_law.angular_velocity: angular_velocity_vector,
    centripetal_law.position_vector: element_position_vector,
})

element_centrifugal_acceleration_vector = solve_for_vector(
    centrifugal_law.law,
    centrifugal_law.centrifugal_acceleration,
).subs(
    centrifugal_law.centripetal_acceleration,
    element_centripetal_acceleration_vector,
)

centrifugal_force_acting_on_element = force_from_acceleration_expr.subs({
    force_law.acceleration: element_centrifugal_acceleration_vector,
    force_law.mass: element_mass_expr,
})

centrifugal_torque_acting_on_element = solve_for_vector(
    torque_def.law,
    torque_def.torque,
).subs({
    torque_def.force: centrifugal_force_acting_on_element,
    torque_def.position_vector: element_position_vector,
})
...
total_torque_vector_acting_on_rod = as_coordinate_vector(gravity_torque_acting_on_rod +
