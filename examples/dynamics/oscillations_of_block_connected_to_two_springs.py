...
total_stiffness = solve(
    stiffness_law.law, stiffness_law.total_stiffness
)[0].subs({
...
period = solve(
    spring_period_law.law, spring_period_law.oscillation_period
)[0].subs({
    spring_period_law.object_mass: mass_sym,
    spring_period_law.spring_elasticity: total_stiffness,
})

frequency = solve(
    frequency_def.law, frequency_def.temporal_frequency
)[0].subs(
    frequency_def.period, period,
).simplify()
...
print(f"The body and two springs system oscillates with a linear frequency of {frequency_value} Hz.")
