...
optical_distance_difference_value = solve(optical_distance_difference_eq,
...
coordinate_value = solve(maximum_condition_eq, two_slits_law.coordinate,
    dict=True)[0][two_slits_law.coordinate]

coordinate_for_first_maximum_value = coordinate_value.subs({number_of_maximum: 1})
coordinate_for_second_maximum_value = coordinate_value.subs({number_of_maximum: 2})
...
print(
    f"distance between first and second maxima is: {print_expression(convert_to(Quantity(distance_between_maxima_), prefixes.milli * units.meters).evalf(3))} mm"
)
